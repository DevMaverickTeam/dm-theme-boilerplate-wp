...
if ( ! function_exists( 'dm_theme_boilerplate_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function dm_theme_boilerplate_posted_on() {
...
if ( ! function_exists( 'dm_theme_boilerplate_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function dm_theme_boilerplate_entry_footer() {
 ...
 * @return bool
 */
function dm_theme_boilerplate_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'dm_theme_boilerplate_categories' ) ) ) {
		...
		set_transient( 'dm_theme_boilerplate_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so dm_theme_boilerplate_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so dm_theme_boilerplate_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in dm_theme_boilerplate_categorized_blog.
 */
function dm_theme_boilerplate_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'dm_theme_boilerplate_categories' );
}
add_action( 'edit_category', 'dm_theme_boilerplate_category_transient_flusher' );
add_action( 'save_post',     'dm_theme_boilerplate_category_transient_flusher' );


if ( ! function_exists( 'dm_theme_boilerplate_comment' ) ) :
    /**
     * Template for comments and pingbacks.
     *
     * Used as a callback by wp_list_comments() for displaying the comments.
     */
    function dm_theme_boilerplate_comment( $comment, $args, $depth ) {
...
endif; // ends check for dm_theme_boilerplate_comment()
